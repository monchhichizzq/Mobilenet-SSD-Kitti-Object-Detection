...
def _depthwise_conv_block(inputs, with_bias, no_bn, pointwise_conv_filters, is_relu6,
                          depth_multiplier=1, strides=(1, 1), block_name='loc'):
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, with_bias, no_bn, is_relu6, block_id=1):
    """
     separable_conv2d
    ...
    if block_id:
        ...
        x = Conv2D(int(expansion * in_channels),
                                  kernel_size=1,
                                  padding='same',
                                  use_bias=with_bias,
                                  activation=None,
...
def conv2d_bn(conv_ind, x, filters, num_row, num_col, padding='same', stride=1, dilation_rate=1, relu=True, is_relu6=True, with_bias=False, no_bn=False):
    x = Conv2D(
...
def mobilenetv2(input_tensor, with_bias, no_bn, is_relu6=True, alpha=1.0):
    net = {}
    # Block 1
    # 160, 480, 3 -> 160, 480, 3
    x = input_tensor
    ...
    # 80, 240, 32 -> 80, 240, 16
    x = _inverted_res_block(x, filters=16, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=1, block_id=0)

    # 80, 240, 16 -> 80, 240, 96 -> 40, 120, 96 -> 40, 120, 24 (no add)
    x = _inverted_res_block(x, filters=24, alpha=alpha, stride=2, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=1)
    # 40, 120, 24 -> 40, 120, 144 -> 40, 120, 24
    x = _inverted_res_block(x, filters=24, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=2)

    #  40, 120, 24- > 40, 120, 32 -> 40, 120, 144 -> 20, 60, 144 -> 20, 60, 32(no add)
    x = _inverted_res_block(x, filters=32, alpha=alpha, stride=2, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=3)
    # 20, 60, 32 -> 20, 60, 192 -> 20, 60, 32
    x = _inverted_res_block(x, filters=32, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=4)
    # 20, 60, 32 -> 20, 60, 192 -> 20, 60, 32
    x, expand_out_1 = _inverted_res_block(x, filters=32, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=5)
    net['conv4_3'] = expand_out_1 #v1 20,60,256

    # 20, 60, 32 -> 20, 60, 64 -> 20, 60, 192 -> 10, 30, 192 -> 10, 30, 64 (no add)
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=2, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=6)
    # 10, 30, 64 -> 10, 30, 384 -> 10, 30, 64
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=7)
    # 10, 30, 64 -> 10, 30, 384 -> 10, 30, 64
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=8)
    # 10, 30, 64 -> 10, 30, 384 -> 10, 30, 64
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=9)

    #  10, 30, 64 -> 10, 30, 384 -> 10, 30, 96 (no add)
    x = _inverted_res_block(x, filters=96, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=10)
    #  10, 30, 96 -> 10, 30, 576 -> 10, 30, 96
    x = _inverted_res_block(x, filters=96, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=11)
    #  10, 30, 96 -> 10, 30, 576 -> 10, 30, 96
    x, expand_out_2 = _inverted_res_block(x, filters=96, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=12)
    net['fc7'] = expand_out_2 # v1 10,30,1024  first port: 576

    # x = Dropout(0.5, name='drop7')(x)
    # Block 6
    # 10, 30, 96 ->  10, 30, 576 -> 5, 15, 576 -> 5, 15, 160 (no add)
    x = _inverted_res_block(x, filters=160, alpha=alpha, stride=2, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=13)
    # 5, 15, 160 -> 5, 15, 960 -> 5, 15, 160
    x = _inverted_res_block(x, filters=160, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=14)
    # 5, 15, 160 -> 5, 15, 960 -> 5, 15, 160
    x = _inverted_res_block(x, filters=160, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=15)
    # 5, 15, 160 -> 5, 15, 960 -> 5, 15, 320  (no add)
    x = _inverted_res_block(x, filters=320, alpha=alpha, stride=1, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=6, block_id=16)
    ...
    # Block 7
    #  5, 15, 1280 -> 3, 8, 512
    net['conv7_2'] = _inverted_res_block(net['conv6_2'], filters=512, alpha=alpha, stride=2, with_bias=with_bias,
                            no_bn=no_bn, is_relu6=is_relu6, expansion=0.2, block_id=17)
    # Block 8
    # 3, 8, 256 -> 2, 4, 256
    net['conv8_2'] = _inverted_res_block(net['conv7_2'], filters=256, alpha=alpha, stride=2, with_bias=with_bias,
                                         no_bn=no_bn, is_relu6=is_relu6, expansion=0.25, block_id=18)

    # Block 9
    # 2, 4, 256 -> 1, 2, 256
    net['conv9_1'] = _inverted_res_block(net['conv8_2'], filters=256, alpha=alpha, stride=2, with_bias=with_bias,
                                         no_bn=no_bn, is_relu6=is_relu6, expansion=0.5, block_id=19)
    # 1, 2, 256 -> 1, 1, 64
    net['conv9_2'] = _inverted_res_block(net['conv9_1'], filters=64, alpha=alpha, stride=2, with_bias=with_bias,
                                         no_bn=no_bn, is_relu6=is_relu6, expansion=0.5, block_id=20)
    return net



def mobilenetv2_ssd(input_shape, is_relu6=True, num_classes=9, with_bias=True, no_bn=False):
    input_tensor = Input(shape=input_shape)
    img_size = (input_shape[1], input_shape[0])

    # Backbone
    net = mobilenetv2(input_tensor, with_bias=with_bias, no_bn=no_bn, is_relu6=is_relu6)
    print('Bulid the SSD achitecture with BN layers')
    ...
    net['conv4_3_norm_mbox_loc'] = _depthwise_conv_block(inputs = net['conv4_3_norm'],
                                                         with_bias = with_bias,
                                                         no_bn = no_bn,
                                                         pointwise_conv_filters = num_priors * 4,
                                                         is_relu6 = is_relu6,
                                                         depth_multiplier=1,
                                                         strides=(1, 1),
                                                         block_name='loc_43')
    ...
    net['conv4_3_norm_mbox_conf'] = _depthwise_conv_block(inputs = net['conv4_3_norm'],
                                                         with_bias = with_bias,
                                                         no_bn = no_bn,
                                                         pointwise_conv_filters = num_priors * num_classes,
                                                         is_relu6 = is_relu6,
                                                         depth_multiplier=1,
                                                         strides=(1, 1),
                                                         block_name='conf_43')
    ...
    net['fc7_mbox_loc'] =  _depthwise_conv_block(inputs = net['fc7'],
                                                 with_bias = with_bias,
                                                 no_bn = no_bn,
                                                 pointwise_conv_filters = num_priors * 4,
                                                 is_relu6 = is_relu6,
    ...
    # num_priors表示每个网格点先验框的数量，num_classes是所分的类
    net['fc7_mbox_conf'] =  _depthwise_conv_block(inputs = net['fc7'],
                                                 with_bias = with_bias,
                                                 no_bn = no_bn,
                                                 pointwise_conv_filters = num_priors * num_classes,
                                                 is_relu6 = is_relu6,
    ...
    # 预测框的处理
    # num_priors表示每个网格点先验框的数量，4是x,y,h,w的调整
    net['conv6_2_mbox_loc'] =  _depthwise_conv_block(inputs = net['conv6_2'],
                                                 with_bias = with_bias,
                                                 no_bn = no_bn,
                                                 pointwise_conv_filters = num_priors * 4,
                                                 is_relu6 = is_relu6,
    ...
    # num_priors表示每个网格点先验框的数量，num_classes是所分的类
    net['conv6_2_mbox_conf'] =  _depthwise_conv_block(inputs = net['conv6_2'],
                                                 with_bias = with_bias,
                                                 no_bn = no_bn,
                                                 pointwise_conv_filters = num_priors * num_classes,
                                                 is_relu6 = is_relu6,
    ...
    # 预测框的处理
    # num_priors表示每个网格点先验框的数量，4是x,y,h,w的调整
    net['conv7_2_mbox_loc'] =  _depthwise_conv_block(inputs = net['conv7_2'],
                                                 with_bias = with_bias,
                                                 no_bn = no_bn,
                                                 pointwise_conv_filters = num_priors * 4,
                                                 is_relu6 = is_relu6,
    ...
    # num_priors表示每个网格点先验框的数量，num_classes是所分的类
    net['conv7_2_mbox_conf'] =  _depthwise_conv_block(inputs = net['conv7_2'],
                                                 with_bias = with_bias,
                                                 no_bn = no_bn,
                                                 pointwise_conv_filters = num_priors * num_classes,
                                                 is_relu6 = is_relu6,
    ...
    # 预测框的处理
    # num_priors表示每个网格点先验框的数量，4是x,y,h,w的调整
    net['conv8_2_mbox_loc'] =  _depthwise_conv_block(inputs = net['conv8_2'],
                                                 with_bias = with_bias,
                                                 no_bn = no_bn,
                                                 pointwise_conv_filters = num_priors * 4,
                                                 is_relu6 = is_relu6,
    ...
    # num_priors表示每个网格点先验框的数量，num_classes是所分的类
    net['conv8_2_mbox_conf'] = _depthwise_conv_block(inputs=net['conv8_2'],
                                                    with_bias=with_bias,
                                                    no_bn=no_bn,
                                                    pointwise_conv_filters=num_priors * num_classes,
                                                    is_relu6=is_relu6,
    ...
    # 预测框的处理
    # num_priors表示每个网格点先验框的数量，4是x,y,h,w的调整
    net['conv9_2_mbox_loc'] =  _depthwise_conv_block(inputs = net['conv9_2'],
                                                 with_bias = with_bias,
                                                 no_bn = no_bn,
                                                 pointwise_conv_filters = num_priors * 4,
                                                 is_relu6 = is_relu6,
    ...
    # num_priors表示每个网格点先验框的数量，num_classes是所分的类
    net['conv9_2_mbox_conf'] = _depthwise_conv_block(inputs=net['conv9_2'],
                                                    with_bias=with_bias,
                                                    no_bn=no_bn,
                                                    pointwise_conv_filters=num_priors * num_classes,
                                                    is_relu6=is_relu6,
...
def load_mobilenetv2_ssd(input_shape=(160, 480, 3),
                        is_relu6=True,
                        num_classes=9,
                        with_bias=False,
                        no_bn=False,
                        pretrained = False):
    model = mobilenetv2_ssd(input_shape=input_shape,
                            is_relu6=is_relu6,
...
if __name__ == '__main__':
    ...
    net = mobilenetv2(input_tensor, with_bias=False, no_bn=False, is_relu6=True)
    ...
    # """
    # tf_model = tf.keras.applications.MobileNetV2(input_shape=(160, 480, 3), alpha=1.0, include_top=False, weights='imagenet')
    # tf_model.summary()
    # mobilenetv2_1_224 = tf_model.get_weights()
    #
    # model_w = model.get_weights()
    ...
    # for i, w in enumerate(model_w):
    #     model_w[i] = mobilenetv2_1_224[i]
    #     print('SSD: {}, mobilenetv2: {}'.format(np.shape(w), np.shape(mobilenetv2_1_224[i])))
    #     if i >= len(mobilenetv2_1_224)-1:
    #         break
    #
    # model.set_weights(model_w)
